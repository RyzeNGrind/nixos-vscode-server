...
- **Purpose:** NixOS module enabling VS Code Server (and VSCodium Server) on NixOS hosts, allowing remote SSH development from VS Code/Codium/Void Editor clients. Critical for the AI village's remote-dev workflow — enables coding directly on NixOS compute nodes from WSL, macOS, or any SSH client.
- **Stack:** Nix NixOS module, systemd service, VS Code Server binary auto-install
- **Active branch:** `combined-prs/83+88` — combined upstream PRs, needs review and merge
